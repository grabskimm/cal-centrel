...
export interface BookingRequest {
  ...
  name: string;
  subject: string;
  meeting: 'none' | 'teams' | 'zoom';
}

/** Validate + normalise an incoming booking request (pure). */
export function validateBooking(
  payload: Record<string, unknown>,
  opts: { defaultSubject: string; zoomAvailable: boolean; nowMs: number },
  ...
  if (meeting !== 'teams' && meeting !== 'zoom') meeting = 'none';
  if (meeting === 'zoom' && !opts.zoomAvailable) meeting = 'none';

  return { ok: true, booking: { start, end, email, name, subject, meeting: meeting as BookingRequest['meeting'] } };
}
...
export function buildGraphEvent(b: BookingRequest, opts: { zoomLink?: string }): Record<string, unknown> {
